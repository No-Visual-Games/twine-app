...
export interface AddTagButtonProps {
    ...
    existingTags: string[];
    /**
     * If there are no existing tags hides the select
     */
    hideIfEmptyExistingTags?: boolean;
    /**
     * Icon for the button.
     */
...
export const AddTagButton: React.FC<AddTagButtonProps> = props => {
    const {assignedTags, disabled, existingTags, icon, label, onAdd, hideIfEmptyExistingTags} = props;
    ...
    const shouldHideAddTagSelect = hideIfEmptyExistingTags && !existingTags.length
    ...
    return (
        <span className="add-tag-button">
            ...
            >
				<CardContent>
					{!shouldHideAddTagSelect && <TextSelect
